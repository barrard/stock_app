...
class Techan_Chart_Basic extends React.Component {
  ...
  componentDidMount() {
    console.log('onmount')
    this.setState({
      svg:d3.select(`#${this.props.divID}`).append("svg")
    })
    // this.setState({svg : d3.select(`#${this.props.divID}`).append("svg")})
    this.props.socket.on(`new_minutley_bar`, this.recieve_new_minute_data);
  }
  ...
  render() {
    console.log('reander chart')
    if(!this.state.svg) return <>hmmmm</>

    console.log(this.state)
    if (typeof d3 == "undefined") return <div>NO D3</div>;
    ...
    let svg = this.state.svg.attr("width", dim.width).attr("height", dim.height);
    ...
    let data = this.props.data[this.props.symbol].chart_data
      .map(function(d) {
        return {
    ...
    return <svg id={`${this.props.symbol}_commodity_chart`}></svg>;
  }
}
