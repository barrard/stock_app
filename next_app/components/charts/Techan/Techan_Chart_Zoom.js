...
class Techan_Chart_Basic extends React.Component {
  ...
  componentDidMount() {
    ...
    var x = techan.scale.financetime().range([0, plot_width]);
  ...
  zoomed() {
    let { y, x, zoomableInit, yAxis, candlestick, yInit } = this.state;
  ...
  redraw_chart() {
    ...
    this.state.zoomableInit = x
      .zoomable().clamp(false)
      .domain([0, data.length])
      .copy();
    this.state.yInit = y.copy();
  }
